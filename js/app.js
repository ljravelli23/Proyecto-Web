...
document.addEventListener('click', event => {
    ...
    if (event.target.closest('#demo-toggle-btn') || event.target.closest('[data-activate-demo]')) {
        toggleDemo();
    }
});
...
function toggleDemo() {
    if (AppState.demoEnabled) {
        ...
        enableDemoData();
        showToast('Datos demo activados: 4 selecciones y 48 barajitas disponibles.');
    }
    renderAll(filter.value);
}
